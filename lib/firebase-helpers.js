...
/**
 * Enregistre un buzz atomiquement (transaction) - seul le premier joueur peut buzzer.
 * Logique simple : le premier buzz qui arrive au serveur gagne.
 * @param {Object} db - Instance Firestore
 * @param {string} playerId - ID du joueur
 * @param {number} localTimestampMs - Timestamp local (ignoré, conservé pour compatibilité API)
 * @returns {Promise<{ok: boolean, reason?: string}>}
 */
export async function registerBuzzerPress(db, playerId, localTimestampMs) {
  ...
  const result = await runTransaction(db, async (tx) => {
    ...
    // Si déjà verrouillé, refuser immédiatement
    ...
    // Verrouiller le buzzer avec ce joueur (premier arrivé au serveur gagne)
    tx.update(stateRef, {
...
/**
 * Réinitialise l'état du buzzer et nettoie la collection temporaire.
 * @param {Object} db - Instance Firestore
...
export async function resetBuzzerState(db, nextState = BUZZER_STATES.IDLE) {
  ...
  // Nettoyer la collection temporaire des tentatives
  // (fait de manière asynchrone pour ne pas bloquer)
