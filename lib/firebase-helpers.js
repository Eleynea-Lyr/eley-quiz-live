...
/**
 * Enregistre un buzz dans la fenêtre d'équité (~80 ms).
 * Le premier buzz ouvre la fenêtre ; les suivants dans la fenêtre sont candidats.
 * Le gagnant est tiré au sort ensuite via resolveBuzzFairWindow (pas le 1er RTT).
 *
 * @param {Object} db - Instance Firestore
 * @param {string} playerId - ID du joueur
 * @param {number} openSeq - Numéro de session d'ouverture du buzzer
 * @returns {Promise<{ok: boolean, reason?: string, windowSeq?: number, openSeq?: number}>}
 */
export async function registerBuzzerPress(db, playerId, openSeq) {
  ...
  try {
    return await runTransaction(db, async (tx) => {
      ...
      const sameWindow =
        Number(data.buzzWindowOpenSeq) === openSeq &&
        Array.isArray(data.buzzCandidateIds) &&
        data.buzzCandidateIds.length > 0;

      let windowSeq = Number(data.buzzWindowSeq) || 0;
      let candidates = sameWindow ? [...data.buzzCandidateIds] : [];
      if (!sameWindow) {
        windowSeq += 1;
        candidates = [];
      }
      ...
      tx.update(stateRef, {
        buzzWindowSeq: windowSeq,
        buzzWindowOpenSeq: openSeq,
        buzzCandidateIds: candidates,
      });

      // Un seul client clôture la fenêtre (le 1er candidat serveur) → évite 2 writes concurrents
      return {
        ok: true,
        windowSeq,
        openSeq,
        shouldResolve: candidates[0] === playerId,
      };
    });
  } catch (e) {
...
/**
 * Clôture la fenêtre d'équité : tire un gagnant au hasard parmi buzzCandidateIds.
 * Plusieurs clients peuvent appeler (transaction idempotente).
 */
export async function resolveBuzzFairWindow(db, openSeq, windowSeq) {
  if (!db || !Number.isFinite(openSeq) || !Number.isFinite(windowSeq)) {
  ...
  try {
    return await runTransaction(db, async (tx) => {
      ...
      if (data.firstPlayerId) {
        return { ok: true, reason: "already-resolved", winnerId: data.firstPlayerId };
      }
      if (Number(data.buzzerOpenSeq) !== openSeq) {
        return { ok: false, reason: "stale-open-seq" };
      }
      if (Number(data.buzzWindowSeq) !== windowSeq) {
        return { ok: false, reason: "stale-window" };
      }
      if (Number(data.buzzWindowOpenSeq) !== openSeq) {
        return { ok: false, reason: "stale-window-open" };
      }
      if (data.buzzerState !== BUZZER_STATES.OPEN) {
        return { ok: false, reason: "buzzer-not-open" };
      }

      const ids = Array.isArray(data.buzzCandidateIds)
        ? data.buzzCandidateIds.filter((id) => typeof id === "string" && id)
        : [];
      if (!ids.length) {
      ...
      return { ok: true, winnerId, candidateCount: ids.length };
    });
  } catch (e) {
    console.error("[resolveBuzzFairWindow] transaction failed:", e);
    return { ok: false, reason: "transaction-failed" };
  }
}
...
/**
 * Recovery live EleyBuzz : tous les joueurs en bleu, buzzer rouvert,
 * invalidation des taps en vol + kick/reload des pages Player figées.
 * @param {Object} db - Instance Firestore
 * @returns {Promise<void>}
 */
export async function recoverEleyBuzzPlayers(db) {
  if (!db) {
    throw new Error("[recoverEleyBuzzPlayers] Missing db");
  }
  ...
  await updateDoc(
    stateRef,
    {
      ...
      playerReloadSeq: increment(1),
      buzzCandidateIds: [],
      buzzWindowOpenSeq: null,
    },
    { merge: true }
  );
}
