// /lib/firebase.js
import { initializeApp, getApps, getApp } from 'firebase/app';
import { getFirestore } from 'firebase/firestore';
import { getStorage } from 'firebase/storage'; // ✅ ajout
import { getAuth } from 'firebase/auth'; // ✅ authentification (joueurs anonymes + admin email/mdp)

const firebaseConfig = {
  apiKey: "AIzaSyD-sJeZyugrh_xUkCzQ2xBQzKy3WklYiTw",
  authDomain: "quizlive-892d0.firebaseapp.com",
  projectId: "quizlive-892d0",
  storageBucket: "quizlive-892d0.firebasestorage.app",
  messagingSenderId: "736686423494",
  appId: "1:736686423494:web:bfb83318ce8dfe374586c7"
};

const app = getApps().length ? getApp() : initializeApp(firebaseConfig);

// Base de données Firestore
export const db = getFirestore(app);

// ✅ Espace fichiers (images des questions)
export const storage = getStorage(app);

// ✅ Authentification (sera utilisée par les joueurs en anonyme et par l'admin en email/mot de passe)
export const auth = getAuth(app);
