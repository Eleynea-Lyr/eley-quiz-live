...
function MiniPlayerBadge({ highlight = "player" }) {
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <span style={{ opacity: dimPlayer ? 0.38 : 1 }}>👤 Toi</span>
      <span
        style={{
          fontVariantNumeric: "tabular-nums",
          opacity: dimPlayer ? 0.38 : 1,
        }}
      >
        • 18
      </span>
      <span
        style={{
          color: dimBuzz ? "rgba(254, 237, 106, 0.35)" : BRAND.yellow,
          opacity: dimBuzz ? 0.55 : 1,
        }}
      >
        ⚡ 5
      </span>
...
function InfoIcon({ size = 22 }) {
  // « i » rond style panneau info (plus joli qu’un italique)
  return (
    <svg
      width={size}
      height={size}
      ...
      style={{ display: "block" }}
    >
      <circle cx="12" cy="12" r="10" fill="none" stroke="rgba(255,251,245,0.92)" strokeWidth="1.75" />
      <circle cx="12" cy="7.6" r="1.35" fill="rgba(255,251,245,0.95)" />
      <path
        d="M12 10.6v6.2"
        fill="none"
...
function PageBody({ page }) {
  ...
  if (page === 0) {
    return (
      <>
        <h2 style={titleStyle}>{m.page1Title}</h2>
        <p style={bodyStyle}>{m.page1Body}</p>
        <div style={{ margin: "14px 0 8px" }}>
          <MiniTeamBadge />
        </div>
        <p style={captionStyle}>{m.page1Caption}</p>
  ...
  return (
    <>
      <h2 style={titleStyle}>{m.page3Title}</h2>
      <p style={bodyStyle}>{m.page3Lead}</p>
      <p style={bodyStyle}>{m.page3Body}</p>
      <div style={{ margin: "14px 0 8px" }}>
        <MiniPlayerBadge highlight="buzz" />
      </div>
      <p style={captionStyle}>{m.page3Caption}</p>
      <BuzzLegend />
      <p style={{ ...bodyStyle, marginTop: 14 }}>{m.page3Close}</p>
    </>
  );
}
    ...
    <>
      <button
        ...
        style={{
          ...
          height: 46,
          borderRadius: "50%",
          border: "1.5px solid rgba(255, 251, 245, 0.38)",
          background:
            "radial-gradient(circle at 35% 30%, rgba(93, 24, 60, 0.95) 0%, rgba(13, 5, 37, 0.92) 70%)",
          color: PAGE_TEXT,
          cursor: open ? "default" : "pointer",
          opacity: open ? 0.28 : 0.88,
          ...
          padding: 0,
          boxShadow:
            "0 4px 14px rgba(0, 0, 0, 0.45), inset 0 1px 0 rgba(255, 251, 245, 0.18)",
