...
/**
 * Récupère le mode de matching pour une question (texte libre uniquement)
 */
export function getAnswerMode(q) {
  return q?.matchingMode || q?.answerMode || q?.matchMode || "relaxed";
}
...
/**
 * Validation du nom d'équipe (max 18 caractères, conversion en majuscules)
 */
export function validateTeamName(raw) {
  if (!raw || typeof raw !== "string") return { ok: false, reason: "empty" };

  const cleaned = raw.replace(/\s+/g, " ").trim();
  if (cleaned.length < 1 || cleaned.length > 18) {
    return { ok: false, reason: "length" };
  }
  if (!NAME_ALLOWED_RE.test(cleaned)) {
    return { ok: false, reason: "charset" };
